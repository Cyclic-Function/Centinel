...
def get_args():
    ...
    parser.add_argument('--gym-num-agents', type=int, default=None)
    parser.add_argument('--gym-max-error-radius', type=float, default=None)
    parser.add_argument('--gym-max-error-velocity', type=float, default=None)
    parser.add_argument('--gym-termination-reward', type=float, default=None)
    parser.add_argument('--gym-target-manager', type=str, default=None)
    parser.add_argument('--gym-agent-target-weights', default=None)
    parser.add_argument('--gym-reward-type', type=str, default=None)
    parser.add_argument('--gym-reward-split', default=None)
    parser.add_argument('--gym-max-steps', type=int, default=None)
    parser.add_argument('--gym-env-type', type=str, default=None)
...
def get_gym_attrs(args: argparse.Namespace = get_args()) -> Dict[str, any]:
    gym_attrs = {}
    for ar, value in vars(args).items():
        if ar[:4] == 'gym_' and value is not None:
            gym_attrs[ar[4:]] = value
    return gym_attrs

if __name__ == "__main__":
    args = get_args()
    gym_attrs = get_gym_attrs(args)
