...
class WitsenhausenCartPole:
    """
    Dual noise: both x and theta has noise
    """
    ...
    def __init__(self, np_random, metadata: Dict[str, Any], attrs: Dict[str, any], render_mode: Optional[str] = None):
        ...
        theta_noise_sd = attrs.get('theta_noise_sd', self.theta_threshold_radians/(4*5))
        x_noise_sd = attrs.get('x_noise_sd', self.x_threshold/(4*5))
        self.agent_strong_theta_noise = self.np_random.normal(loc=0.0, scale=theta_noise_sd)
        self.agent_strong_x_noise = self.np_random.normal(loc=0.0, scale=x_noise_sd)
    ...
    def observe(self, agent):
        ...
        if agent == 'agent_weak':
            ...
            noise = np.array(
                [self.agent_strong_x_noise, 0,
                 self.agent_strong_theta_noise, 0],
                dtype=np.float32
            )
