...
class WitsenhausenCartPole:
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, np_random, metadata: Dict[str, Any], attrs: Dict[str, any], render_mode: Optional[str] = None):
        ...
        self.k = attrs.get('k', 0.05)    # Witsenhausen parameter
        ...
        if not terminated:
            if agent == "agent_weak":
                if self.reward_mode == 'train':
                    # scale it so its not too small, normalise by max steps
                    reward = (self.survival_bonus - theta**2 - (self.k**2)*(force**2)*self.tau)*self.reward_scale/self.max_steps      # TODO: should this be abs?
                elif self.reward_mode == 'test':
                    # no survival bonus
                    reward = (-theta**2 - (self.k**2)*(force**2)*self.tau)*self.reward_scale/self.max_steps
