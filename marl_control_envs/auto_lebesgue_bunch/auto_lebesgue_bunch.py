...
class AutoLebesgueBunch:
    ...
    def __init__(self, np_random, gym_attrs, metadata, render_mode=None, test_reward=False):
        valid_gym_attrs = (
            ...
            'max_steps', 'env_type', 'test_reward_flags',
        )
        ...
        self.max_action = np.array(
            [1.0, 1.0, np.finfo(np.float32).max],
            dtype=np.float32
        )
        self.action_spaces = {
            i: spaces.Box(
        ...
        all_agent_local_target_high = np.resize(local_target_high, len(local_target_high)*self.num_agents)
        all_agent_high = np.resize(single_agent_high, len(single_agent_high)*self.num_agents)
        obs_high = np.concatenate([all_agent_local_target_high, all_agent_high])
        ...
        self.single_agent_max_steps = gym_attrs.get('max_steps', 300)
        self.max_steps = self.single_agent_max_steps*self.num_agents
    ...
    def observe(self, agent):
        ...
        obs = np.concatenate([current_local_target, other_agent_local_targets, current_agent_state, other_agent_states], dtype=np.float32)
    ...
    def step(self, action, agent, action_liars_mask=None):
        ...
        action = action[0:2]
        action_liars_mask = action_liars_mask[0:2]
        ...
        self.rewards[agent] += -(1000.0/honesty)/self.single_agent_max_steps
        ...
        if not terminated:
            if not (self.test_reward and ('0_honesty' in self.test_reward_flags or 'mask_all_actions' in self.test_reward_flags)):
                ...
                self.rewards[agent] += -honesty*np.linalg.norm(action - action_liars_mask)
