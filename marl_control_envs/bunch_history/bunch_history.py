...
import marl_control_envs.bunch_history.target_managers as tm
...
class BunchHistory:
    ...
    def __init__(self, np_random, gym_attrs, metadata, render_mode=None, test_reward=False):
        ...
        valid_gym_attrs = (
            ...
            'max_steps', 'env_type', 'history_length',
        )
        ...
        self.history_length = gym_attrs.get('history_length', 2)
        self.history_agents = {i:None for i in self.agents}
        ...
        all_agent_high = np.resize(single_agent_high, len(single_agent_high)*self.num_agents*self.history_length)
    ...
    def reset(self):
        ...
        for i in self.agents:
            ...
            self.history_agents[i] = np.tile(self.finder_agents[i].get_pos(), self.history_length)
    ...
    def observe(self, agent):
        ...
        current_agent_state = self.history_agents[agent]
        other_agent_states = np.concatenate([self.history_agents[i] for i in self.agents if i != agent])
        
        obs = np.concatenate([local_target, current_agent_state, other_agent_states], dtype=np.float32)
        
        return obs
    
    def _update_history(self, agent):
        num_coords = 2
        self.history_agents[agent] = np.roll(self.history_agents[agent], num_coords)
        self.history_agents[agent][:num_coords] = self.finder_agents[agent].get_pos()
