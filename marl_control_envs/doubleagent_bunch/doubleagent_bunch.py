...
class DoubleAgentBunch:
    class DoubleAgent:
        ...
        def reset(self):
            ...
            # idk what a better thing to do would be
        
        def update_pos_estimate(self, estimated_pos):
    ...
    def __init__(self, np_random, gym_attrs, metadata, render_mode=None, test_reward=False):
        ...
        # TODO: avoid self.num_agents and self.agents in this class!
        ...
        all_agents_standard_high = np.resize(agent_standard_high, len(agent_standard_high)*self.num_agents_standard)
        obs_high = np.concatenate([local_target, all_agents_standard_high, agent_double_high])
        ...
        self.finder_agents = {i: self.FinderAgent(self.np_random) for i in self.agents_standard}
        self.finder_agents[self.agent_double] = self.DoubleAgent()
        
        '''
        WARNING
        -------
        self.finder_agents[self.agent_double] is a dummy class! Ignore it!
        '''
        
        target_manager_type = gym_attrs['target_manager']
        if target_manager_type == 'DoubleAgentManager2D':
            self.target_manager = tm.DoubleAgentManager2D(self.np_random, self.agents)
        else:
    ...
    def observe(self, agent):
        ...
        all_standard_agent_states = np.concatenate([self.finder_agents[i].state for i in self.agents_standard])
        agent_double_state = self.finder_agents[self.agent_double].get_estimated_pos()
        obs = np.concatenate([local_target, all_standard_agent_states, agent_double_state])
    ...
    def step(self, action, agent):
        assert self.agent == self.agent_double or self.finder_agents[agent].state is not None, "Call reset before using step method."
        
        action = np.clip(action, -self.pos_max, self.pos_max)
        if agent in self.agents_standard:
            self.finder_agents[agent].update_state(action)
        elif agent == self.agent_double:
            self.finder_agents[agent].update_pos_estimate(action)
        
        global_target = self.target_manager.global_target
        
        truncated = False
        all_agents_within_max_pos_error = np.all(
            [np.linalg.norm(global_target - self.finder_agents[i].get_coords()) < self.pos_max_error for i in self.agents_standard]
        )
        all_agents_within_max_vel_error = np.all(
            [self.finder_agents[i].get_vel(magnitude=True) < self.vel_max_error for i in self.agents_standard]
        )
        terminated = bool(
        ...
        if not terminated:
            if self.test_reward or self.reward_type == 'cooperative':
                if agent in self.agents_standard:
                    reward = -np.sum([
                        np.linalg.norm(global_target - self.finder_agents[i].get_coords()) for i in self.agents_standard
                    ])
                elif agent == self.agent_double:
                    reward = -np.linalg.norm(global_target - self.finder_agents[agent].get_estimated_pos())
